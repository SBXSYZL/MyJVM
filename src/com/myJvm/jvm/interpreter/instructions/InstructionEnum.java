...
public final class InstructionEnum {
    ...
    public static String findInstructionName(int instruction) {
        instruction &= 0xff;
        String instructionName = map.get(instruction);

        if (instructionName == null) {
            throw new RuntimeException("Find Instruction Error");
        }
        instructionName = instructionName.replace("_", "");
        String instructionHex = Integer.toHexString(instruction & 0xff);
        if (instructionHex.length() == 1) {
            instructionHex = "0" + instructionHex;
        }
        instructionHex = "0x" + instructionHex;
        MyLog.debug(instructionHex + "    " + instructionName);
